...
class Node:
    ...
    - all([self.address in x.endpoint for x in self.channels.values()])
    """
    address: NodeAddress
    ...
    def find_paths(self, destination: NodeAddress, visited: set[Node]) -> list[list[Channel]]:
        ...
        """
        if self.address == destination or self in visited:
            return [[]]
        else:
            visited.add(self)
            possible_paths = []
            neighbour_add = self.channels.keys()
            visited_add = [n.address for n in visited]
            for address in neighbour_add:
                if address not in visited_add:
                    channel = self.channels[address]
                    neighbour = channel.get_other_endpoint(self)
                    paths = neighbour.find_paths(destination, visited)
                    # for p in paths:
                    #     p.append(channel)
                    self.find_paths_helper(channel, paths)
                    possible_paths += paths
            return possible_paths

    def find_paths_helper(self, channel: Channel, paths: list[list[Channel]]) -> None:
        """ Helper method for find_paths """
        for p in paths:
            p.append(channel)
...
class Channel:
    ...
    - self.occupant is None or (self.occupant.next_stop is None or self.occupant.next_step in self.endpoints)
    - all([isinstance(packet, Packet) and packet.next_stop is None or \
        packet.next_stop in self.endpoints for packet in self.buffer])
...
class AbstractNetwork:
    ...
    Representation Invariants:
    - ... # In the _nodes dictionary, each address key is equal to its corresponding Node object’s address attribute
    """
    ...
    def transmit_packet(self, packet: Packet) -> list[NodeAddress]:
        ...
        Implementation Hint:
            - You can use the packet.next_stop attribute to keep track of which nodes the packet
              is visiting
        """
        node_address_path = [packet.source]
        channel = self.add_new_packet(packet)
        while packet.next_stop.address != packet.destination:
            packet.source = packet.next_stop.address
            node_address_path.append(packet.source)
            self.activate_channel(channel)
            channel = self.route_packet(packet.source, packet)
        node_address_path.append(packet.next_stop.address)
        self.activate_channel(channel)
        return node_address_path
